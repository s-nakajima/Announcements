	...
	<p class="container">
		<?php
			foreach ($partList as $key => $item) {
				...
				<p class="container">
				<?php
				if (isset($item[$roomPartKey]['edit_content']) &&
					$item[$roomPartKey]['edit_content'] == 1) {
						echo '<span class="glyphicon glyphicon-ok"></span>';
						echo h($item[$roomPartKey]['name']) . '';
				} elseif ( isset($item[$roomPartKey]['edit_content']) &&
					$item[$roomPartKey]['edit_content'] == 0) {
						echo '<span class="glyphicon glyphicon-remove"></span>';
						echo h($item[$roomPartKey]['name']) . '';
				} elseif (isset($editVariableArray[$partId]) &&
					$editVariableArray[$partId]) {
